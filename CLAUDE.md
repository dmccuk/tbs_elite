...
**Aboard the Kessler** (K on the title screen, or its row; `src/interior/`): walk the hangar deck in first person. A separate scene and camera in **metres** (the flight sim stays in km) replaces the world render passes (`setWorldPassesEnabled` in renderer.ts); `G.phase` stays `"splash"` and `body.walking` hides the flight HUD. WASD walk, Shift run, mouse look, E to use a hotspot. **P/Esc opens the normal pause menu** (RESUME / CONTROLS / SOUND / MISSIONS — RESTART and the steering mode are hidden by `body.walking`, and the help menu swaps its flight sections for an "On foot" one); MISSIONS calls `exitInterior()` on its way back to the title screen. Esc closes the flight-ops menu first if that's open. While paused, `updateInterior` is skipped, so the walker and the doors freeze.

Careful with `G.phase`: it stays `"splash"` the whole time you're on foot, so anything guarded by "are we on the title screen?" must also check `interiorActive()` (`dismissSplash` and `setPaused` in main.ts both do). The flight-ops console opens a menu that starts any flight (the Cruise, landing practice, free flight, the Prologue). Built in sections: **1 = the hangar deck**, **2 = the main corridor + ready room**, **3 = the bridge** (all done); next come the mess hall and quarters, then returning to the deck after a flight. Doors open themselves when you're within ~3 m unless locked; `Section.solids` plus the shut doors' boxes are what the walker collides with. Rooms are built with `RoomKit` in `interior/parts.ts` (shell with doorway gaps, powered doors, shared plating textures); a wall shared by two rooms is built once, by one of them, and `skip`ped by the other.

The bridge is at the forward end of the corridor. The Kessler is a carrier, so her bow is armour and there is no forward glass: the watch conns her off a main viewscreen and a wall of repeaters, and the only real window is the long **port-side viewport** over the belt (stars, the brown dwarf and a slowly drifting rock field, all built out to port where no other section can stray into the view). Stations: helm, nav, sensors, comms, the command dais and the plot table, all with E lines.
...
src/interior/interior.ts  On foot inside the Kessler: its own scene/camera (METRES, not km), rendered in place of the world passes; hotspots, prompts, the flight-ops menu
src/interior/walk.ts    First-person controller: mouse look, WASD at walking pace, circle-vs-box collision so you slide along walls
src/interior/deck.ts    The hangar deck (Section 1): shell, lighting rig, deck markings, the two Seagull cradles, clutter, flight ops console, doorway to the corridor
src/interior/corridor.ts  The main corridor and ready room (Section 2): vestibule, 2.6 m corridor, powered sliding doors, lockers/benches/plan table
src/interior/bridge.ts  The bridge (Section 3): viewscreen wall, watch stations, command dais, plot table, port viewport and the belt outside it
src/interior/parts.ts   RoomKit: room shells with doorway gaps, powered doors, and the shared plating/panel/sign textures
src/interior/types.ts   Shared interior shapes: Hotspot, Door, Section
src/model-kit.ts        Shared model helpers: geometry/material shorthands, glow beacons, truss lattices (Trusses), canvas textures, the part baker
src/models-stations.ts  The Lingering Systems' landmarks: Halvern Ring (spun habitat ring, ~4.9 km) and Cawley Yards (refinery and repair dock, ~5 km)
...
- **Keys:**
  ...
  - P/Esc: pause (on foot aboard the Kessler too — that's the way back to the title screen). H: help. M: mute (saved as `tbs-muted`). Speaker icons on the title screen and at the end of the HUD's instrument row show the state (red with a line through it when muted) and toggle it on click. M on the title screen mutes rather than starting a mission.
